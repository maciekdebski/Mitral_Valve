...
tbl2b <- tbl_summary(Data_PS_Repair[,-(1:2)], by = "Minimitral") %>%
  add_p() %>%
  bold_p()
...
```{r KM Curve (weighted)}
fit1<- survfit(Surv(time = Mortality_FU, event = as.numeric(Dead)) ~ MitralValveProcedure_RepaPepl, data = Data_PS_Nini, weights = weights)
