...
Data_PS_Mort <- Data %>%
  drop_na(MitralValveProcedure_RepaPepl, Age , Gender , BMI , PMHx_HTN, Diabetes_col, NYHA34_12, PrevPCI_col, CardiacSurgery_col, PMHx_PulmonaryDx, Smoking_Stat, PMHx_CVATIA, Canadian_Score4, Critical_Preop, PulmonHTN_bin, AF, LMS_Dx, Extent_of_CorVessDx, Extracardiac_Arteriopathy, Recent_MI, LVEF_Cat, Op_Urgency_col, CreatinineGT200, CABG, Tricuspid_Procedure_Bin, NativeMitralPathology) %>%
...
match.it <- matchit(MitralValveProcedure_RepaPepl ~ Age + Gender + BMI + PMHx_HTN+ Diabetes_col+ NYHA34_12+ PrevPCI_col+ CardiacSurgery_col+ PMHx_PulmonaryDx+ Smoking_Stat+ PMHx_CVATIA+ Canadian_Score4+ Critical_Preop+ PulmonHTN_bin+ AF+ LMS_Dx+ Extent_of_CorVessDx+ Extracardiac_Arteriopathy+ Recent_MI+ LVEF_Cat+ Op_Urgency_col+ CreatinineGT200+ CABG+ Tricuspid_Procedure_Bin+ NativeMitralPathology, data = Data_PS_Mort,  method = "genetic", replace = T, caliper = 0.2, pop.size = 100, ratio = 2)
a <- summary(match.it)
...
UV_T <- gtsummary::tbl_uvregression(Data_PS,
                 ...
                 exponentiate = T,
                 include = c(-id, -subclass, -ID, -all_of(UniLevel), -Dead, -Mortality_FU, -MitralValvePathology, -weights)) %>%
...
fit2<- survfit(Surv(time = Mortality_FU, event = as.numeric(Dead)) ~ NativeMitralPathology, data = Data_PS, weights = weights)
plot2 <- ggsurvplot(fit2, pval = TRUE, conf.int = TRUE,
           risk.table = T, risk.table.y.text.col = TRUE, xscale="d_y", 
           legend.labs = c("Degenerative", "Functional", "Ischaemic"), 
           xlab = "Years Since Index Procedure", ylab = "Survival Probability", palette = c("#fba30c", "#0077b8", "#038271"), theme = "lancet", break.time.by	= 1826.25, pval.coord = c(0,0.3), legend.title = "")

Data_PS_Repair <- Data_PS %>% filter(MitralValveProcedure_RepaPepl == "Repair")

Data_PS_Replace <- Data_PS %>% filter(MitralValveProcedure_RepaPepl == "Replacement")

fit3<- survfit(Surv(time = Mortality_FU, event = as.numeric(Dead)) ~ NativeMitralPathology, data = Data_PS_Repair, weights = weights)
plot3 <- ggsurvplot(fit3, pval = TRUE, conf.int = TRUE,
           risk.table = T, risk.table.y.text.col = TRUE, xscale="d_y", 
           legend.labs = c("Degenerative", "Functional", "Ischaemic"), 
           xlab = "Years Since Index Procedure", ylab = "Survival Probability (Repair)", palette = c("#fba30c", "#0077b8", "#038271"), theme = "lancet", break.time.by	= 1826.25, pval.coord = c(0,0.3), legend.title = "")

fit4<- survfit(Surv(time = Mortality_FU, event = as.numeric(Dead)) ~ NativeMitralPathology, data = Data_PS_Replace, weights = weights)
plot4 <- ggsurvplot(fit4, pval = TRUE, conf.int = TRUE,
           risk.table = T, risk.table.y.text.col = TRUE, xscale="d_y", 
           legend.labs = c("Degenerative", "Functional", "Ischaemic"), 
           xlab = "Years Since Index Procedure", ylab = "Survival Probability (Replacement)", palette = c("#fba30c", "#0077b8", "#038271"), theme = "lancet", break.time.by	= 1826.25, pval.coord = c(0,0.3), legend.title = "")

Data_PS_Degen <- Data_PS %>% filter(NativeMitralPathology == "Degenerative")

Data_PS_Ischae <- Data_PS %>% filter(NativeMitralPathology == "Ischaemic")

Data_PS_Funct <- Data_PS %>% filter(NativeMitralPathology == "Functional regurgitation")

fit5<- survfit(Surv(time = Mortality_FU, event = as.numeric(Dead)) ~ MitralValveProcedure_RepaPepl, data = Data_PS_Degen, weights = weights)
plot5 <- ggsurvplot(fit5, pval = TRUE, conf.int = TRUE,
           risk.table = T, risk.table.y.text.col = TRUE, xscale="d_y", 
           legend.labs = c("Repair", "Replacement"), 
           xlab = "Years Since Index Procedure", ylab = "Survival Probability (Degenerative)", palette = c("#fba30c", "#0077b8"), theme = "lancet", break.time.by	= 1826.25, pval.coord = c(0,0.3), legend.title = "")

fit6 <- survfit(Surv(time = Mortality_FU, event = as.numeric(Dead)) ~ MitralValveProcedure_RepaPepl, data = Data_PS_Ischae, weights = weights)
plot6 <- ggsurvplot(fit6, pval = TRUE, conf.int = TRUE,
           risk.table = T, risk.table.y.text.col = TRUE, xscale="d_y", 
           legend.labs = c("Repair", "Replacement"), 
           xlab = "Years Since Index Procedure", ylab = "Survival Probability (Ischaemic)", palette = c("#fba30c", "#0077b8"), theme = "lancet", break.time.by	= 1826.25, pval.coord = c(0,0.3), legend.title = "")

fit7<- survfit(Surv(time = Mortality_FU, event = as.numeric(Dead)) ~ MitralValveProcedure_RepaPepl, data = Data_PS_Funct, weights = weights)
plot7 <- ggsurvplot(fit7, pval = TRUE, conf.int = TRUE,
           risk.table = T, risk.table.y.text.col = TRUE, xscale="d_y", 
           legend.labs = c("Repair", "Replacement"), 
           xlab = "Years Since Index Procedure", ylab = "Survival Probability (Functional)", palette = c("#fba30c", "#0077b8"), theme = "lancet", break.time.by	= 1826.25, pval.coord = c(0,0.3), legend.title = "")
